...
Two directed-results variants are retained under `notebooks/deprecated/` for sensitivity and development provenance:

- `notebooks/deprecated/04_directed_results_plus3.ipynb` — strict-core sensitivity/development variant.
- `notebooks/deprecated/04_directed_results_top30.ipynb` — top-30 core-window sensitivity/development variant.
...
These CSVs are versioned sensitivity artifacts copied unchanged from historical notebook-root artifacts. They are not raw data. Going forward, the namespaced files under `results/sensitivity/` are the preferred sources for canonical robustness reruns; historical notebook-root copies have been moved unchanged to `results/sensitivity/historical/notebook_root/` for provenance and should not be treated as preferred inputs.
...
- the historical `top30` naming mismatch around `core_v2_top100.pkl`, if still relevant to provenance review;
